...
function serviceBody(v: Record<string, string>): string {
  ...
  return `
    ...
    <p>This Service Agreement (this <strong>"Agreement"</strong>) is made as of ${fill(today())} (the <strong>"Effective Date"</strong>) by and between ${fill(p)} (<strong>"Provider"</strong>) and ${fill(c)} (<strong>"Client"</strong>).</p>
    ${S("1. Services")}<p>Provider agrees to perform the following services (the <strong>"Services"</strong>): ${fill(v.services, "[Services]")}, beginning ${fill(v.startDate, "[Start Date]")}. Provider shall perform the Services in a professional and workmanlike manner consistent with industry standards.</p>
    ${S("2. Term & Termination")}<p>This Agreement begins on the Effective Date and continues until the Services are completed, unless terminated earlier. Either party may terminate for convenience on ten (10) days' written notice, or immediately for the other party's material breach uncured for five (5) days after notice. Client shall pay for Services properly performed through the termination date.</p>
    ${S("3. Fees & Payment")}<p>Client shall pay <strong>${fill(v.fee, "[Fee]")}</strong>, due <strong>${fill(pick(v, "paymentTerms"), "Net 15")}</strong> against Provider's invoices. Past-due amounts accrue interest at the lesser of 1.5% per month or the maximum lawful rate.</p>
    ${S("4. Client Responsibilities")}<p>Client shall provide timely access to information, materials, and approvals reasonably required for Provider to perform the Services. Provider is not responsible for delays caused by Client's failure to do so.</p>
    ${S("5. Independent Contractor")}<p>Provider is an independent contractor and not an employee, partner, or agent of Client. Provider controls the manner and means of performing the Services and is responsible for its own taxes and expenses.</p>
    ${S("6. Ownership of Deliverables")}<p>Upon full payment, all deliverables created specifically for Client under this Agreement become Client's property, and Provider assigns to Client its rights in such deliverables. Provider retains ownership of its pre-existing materials and general know-how, and grants Client a license to use them as incorporated into the deliverables.</p>
    ${S("7. Confidentiality")}<p>Each party shall keep confidential the other's non-public information and use it only to perform under this Agreement. This obligation survives termination.</p>
    ${S("8. Warranty & Disclaimer")}<div class="tpl-highlight">Provider warrants that the Services will be performed in a professional and workmanlike manner. EXCEPT AS EXPRESSLY STATED, THE SERVICES AND DELIVERABLES ARE PROVIDED WITHOUT OTHER WARRANTIES, EXPRESS OR IMPLIED, INCLUDING MERCHANTABILITY OR FITNESS FOR A PARTICULAR PURPOSE.</div>
    ${S("9. Limitation of Liability")}<p>EXCEPT FOR CONFIDENTIALITY OR INDEMNIFICATION OBLIGATIONS, NEITHER PARTY IS LIABLE FOR INDIRECT OR CONSEQUENTIAL DAMAGES, AND EACH PARTY'S TOTAL LIABILITY SHALL NOT EXCEED THE FEES PAID UNDER THIS AGREEMENT.</p>
    ${S("10. Indemnification")}<p>Each party shall indemnify and hold the other harmless from third-party claims arising from its own negligence, willful misconduct, or breach of this Agreement.</p>
    ${S("11. General")}<div class="tpl-highlight"><strong>Governing Law:</strong> the laws of the State of ${fill(st)}, without regard to conflicts of laws.<br><br><strong>Assignment:</strong> neither party may assign this Agreement without the other's prior written consent.<br><br><strong>Entire Agreement:</strong> this is the entire agreement and supersedes prior understandings; amendments must be in writing signed by both parties.<br><br><strong>Counterparts & Electronic Signature:</strong> this Agreement may be executed in counterparts and by electronic signature, each with the same effect as an original.</div>
    ${sig("Provider", fill(p), "Client", fill(c))}`;
}
